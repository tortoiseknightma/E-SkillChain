"""Execute the single approved Codex/high v5 authoring session.

The v5 authority state machine makes terminal evidence exist *before* the
...
import skillchain.codex_authoring_v4 as v4_contract_module
import skillchain.codex_authoring_v5 as contract_module
...
V4_RUNNER_PATH = Path(v4_runner.__file__).resolve(strict=True)
...
V4_CONTRACT_MODULE_PATH = Path(v4_contract_module.__file__).resolve(strict=True)
...
FREEZE_PATH = (
    ROOT / "specs" / "authoring" / "authoring-freeze-lock-codex-high-v5.json"
)
...
V4_CANDIDATE_ID = "authoring-codex-high-20260724-v4"
V4_RUN_ID = "llm-static-codex-primary-20260724-high-v4"


@dataclass(frozen=True)
class V5CappedProcessResult:
    returncode: int
    stdout: bytes
    stderr: bytes
    timed_out: bool
...
def _validate_frozen_source_bindings(
    ...
    expected = {
        ...
        "v4_runner": (V4_RUNNER_PATH, freeze_bindings["v4_runner"][1]),
        ...
        "v4_contract_module": (
            V4_CONTRACT_MODULE_PATH,
            freeze_bindings["v4_contract_module"][1],
        ),
        "v3_contract_module": (
    ...
    for runtime_name, binding in expected.items():
        if v2_runner._runtime_binding(runtime, runtime_name) != binding:
            raise CodexAuthoringContractError(
                f"Codex v5 runtime {runtime_name} binding differs from the freeze"
            )
...
def _validate_dependency_snapshot(manifest: dict[str, Any]) -> None:
    v2_runner._self_hash(
        ...
        "Codex v5 source manifest",
    )
    unsigned = dict(manifest)
    unsigned.pop("source_manifest_payload_sha256", None)
    live = build_codex_runtime_dependency_snapshot_v5(ROOT)
    if canonical_json_bytes(unsigned) != canonical_json_bytes(live):
        raise CodexAuthoringContractError(
            "Codex v5 source or runtime dependency snapshot drifted"
        )
...
def _terminal_guard(
    ...
    *,
    ...
    if set(guard) != set(required) | {"terminal_guard_payload_sha256"}:
        raise CodexAuthoringContractError(
            "Codex v5 conditional terminal guard fields drifted"
        )
    for key, expected in required.items():
        if guard.get(key) != expected:
            raise CodexAuthoringContractError(
                f"Codex v5 conditional terminal guard drifted: {key}"
            )
    return guard
...
def _approval(
    ...
    *,
    ...
    v4_receipt_path, v4_receipt_file_sha256 = v2_runner._binding(
        freeze,
        "v4_canonical_receipt",
    )
    v4_receipt = v2_runner._load_object(
        v4_receipt_path,
        v4_receipt_file_sha256,
        "Codex v4 canonical receipt",
    )
    v2_runner._self_hash(
        v4_receipt,
    ...
    required = {
        ...
        "v4_canonical_receipt_file_sha256": v4_receipt_file_sha256,
        "v4_canonical_receipt_payload_sha256": v4_receipt.get(
            "receipt_payload_sha256"
        ),
    ...
    if set(raw) != set(required) | {"approval_payload_sha256"}:
        raise CodexAuthoringContractError(
            "Codex v5 owner approval contains missing or unexpected fields"
        )
    for key, expected in required.items():
        if raw.get(key) != expected:
            raise CodexAuthoringContractError(
                f"Codex v5 owner approval does not accept frozen field: {key}"
            )
    if (
        ...
        or v4_receipt.get("candidate_id") != V4_CANDIDATE_ID
        or v4_receipt.get("run_id") != V4_RUN_ID
        ...
        raise CodexAuthoringContractError(
            "Codex v2 authorization was not safely retired for v5"
        )
    return raw
...
def _binary_evidence(
    ...
    evidence = v2_runner._load_object(
        path,
        expected_sha256,
        "Codex v5 absolute-binary evidence",
    )
    v2_runner._self_hash(
        evidence,
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 absolute-binary evidence differs from runtime"
        )
...
def _local_freeze_binding(
    ...
    if not isinstance(item, dict) or set(item) != {"file", "file_sha256"}:
        raise CodexAuthoringContractError(
            f"Codex v5 terminal validation binding is invalid: {name}"
        )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            f"Codex v5 terminal validation binding values are invalid: {name}"
        )
    candidate = ROOT / relative
    resolved = candidate.resolve(strict=True)
    if candidate.is_symlink() or ROOT.resolve() not in resolved.parents:
        raise CodexAuthoringContractError(
            f"Codex v5 terminal validation binding is unsafe: {name}"
        )
    content = read_stable_regular_file(
        resolved,
        label=f"Codex v5 terminal validation binding {name}",
    )
    if sha256_bytes(content) != digest:
        raise CodexAuthoringContractError(
            f"Codex v5 terminal validation binding drifted: {name}"
        )
...
def _load_terminal_validation_context(
    *,
    ...
    claim_bytes = read_stable_regular_file(
        claim_path,
        label="Codex v5 authorization claim",
    )
    ...
    if not isinstance(claim, dict) or set(claim) != CLAIM_FIELDS:
        raise CodexAuthoringContractError(
            "Codex v5 authorization claim fields drifted"
        )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 authorization claim semantics drifted"
        )

    freeze = v2_runner._load_object(
        FREEZE_PATH,
        claim["freeze_lock_file_sha256"],
        "Codex v5 freeze lock",
    )
    v2_runner._self_hash(
        freeze,
        "freeze_payload_sha256",
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 freeze differs from the terminal authority chain"
        )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 owner approval reference is unsafe"
        )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 owner approval reference differs from the freeze"
        )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 owner approval differs from the terminal authority chain"
        )
    ...
    for binding_name, claim_field in claim_binding_map.items():
        ...
        if claim.get(claim_field) != bound_digest:
            raise CodexAuthoringContractError(
                f"Codex v5 claim differs from freeze binding: {binding_name}"
            )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 claim input or retirement binding drifted"
        )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 runtime differs from the claim"
        )
...
def _validate_published_bundle(
    ...
    try:
        _real_directory(output_dir, "Codex v5 canonical output directory")
...
def _publish_and_classify_bundle(
    ...
    *,
    ...
    if publish_error is not None:
        raise publish_error
    raise CodexAuthoringContractError(
        "Codex v5 canonical bundle publication could not be validated"
    )
    ...
    *,
    ...
    try:
        ...
        receipt_bytes = read_stable_regular_file(
            receipt_path,
            label="Codex v5 canonical terminal receipt",
        )
        receipt = parse_canonical_json(
            receipt_bytes,
            label="Codex v5 canonical terminal receipt",
        ...
        for name, digest in files.items():
            content = read_stable_regular_file(
                output_dir / name,
                label=f"Codex v5 canonical bundle file {name}",
            )
            if sha256_bytes(content) != digest:
                return False
        evidence_bytes = read_stable_regular_file(
            output_dir / "session-evidence.json",
            label="Codex v5 canonical session evidence",
        )
        ...
        audit = v2_runner._event_audit(
            read_stable_regular_file(
                ...
                label="Codex v5 canonical event log",
            )
        )
        if (
        ...
        if success:
            raw_bytes = read_stable_regular_file(
                ...
                label="Codex v5 canonical raw final message",
            )
            ...
            archived_draft = read_stable_regular_file(
                ...
                label="Codex v5 canonical pre-review draft",
            )
    ...
    if os.path.lexists(path):
        if (
            ...
            or read_stable_regular_file(
                path,
                label=f"Codex v5 staging file {name}",
            )
            != content
        ):
            raise CodexAuthoringContractError(
                f"Codex v5 staging file conflicts: {name}"
            )
        return
...
def main() -> int:
    ...
    v2_runner._self_hash(freeze, "freeze_payload_sha256", "Codex v5 freeze lock")
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 freeze lock is not an approvable candidate"
        )
    ...
    if (
        ...
        or runtime.get("runtime_id") != "codex-cli-author-runtime-20260724-v5"
    ):
        raise CodexAuthoringContractError("Codex v5 runtime lock is not frozen")
    ...
    request_bytes = v2_runner._read_bound_bytes(
        request_path,
        request_sha256,
        "Codex v5 canonical request",
    )
    stdin_bytes = v2_runner._read_bound_bytes(
        stdin_path,
        stdin_sha256,
        "Codex v5 stdin request",
    )
    schema_bytes = v2_runner._read_bound_bytes(
        schema_path,
        schema_sha256,
        "Codex v5 output schema",
    )
    request = parse_codex_authoring_request(request_bytes)
    try:
        raw_schema = parse_canonical_json(
            schema_bytes,
            label="Codex v5 output schema",
        )
    except ArtifactFormatError as error:
        raise CodexAuthoringContractError(
            "Codex v5 output schema is not canonical JSON"
        ) from error
    if not isinstance(raw_schema, dict):
        raise CodexAuthoringContractError(
            "Codex v5 output schema must contain an object"
        )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 packet/request/schema/stdin or budget drifted"
        )
    ...
    if not isinstance(paths, dict) or paths.get("run_id") != CODEX_AUTHOR_RUN_ID:
        raise CodexAuthoringContractError(
            "Codex v5 authorization paths are missing or drifted"
        )
    output_dir = v2_runner._repo_path(
        paths.get("output_directory"),
        "Codex v5 output directory",
        must_exist=False,
    )
    ...
    expected_approval_path = v2_runner._repo_path(
        ...
        "frozen Codex v5 owner approval",
        must_exist=True,
    )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 approval or receipt path differs from the freeze"
        )
    ...
    if (
        ...
        raise CodexAuthoringContractError(
            "Codex v5 terminal guard policy is missing"
        )
    ...
    if any(os.path.lexists(path) for path in (output_dir, claim_path, receipt_path)):
        raise FileExistsError(
            "Codex v5 authorization is consumed or a canonical output path exists"
        )
    ...
    try:
        ...
        if (
            ...
            raise CodexAuthoringContractError(
                "Codex v5 runtime command shape drifted"
            )
    except BaseException:
    ...
    try:
        ...
        try:
            ...
            completed = _run_capped_process_v5(
                command,
                cwd=scratch,
            ...
            if (
                ...
                raise CodexAuthoringContractError(
                    "Codex v5 process did not complete within the frozen envelope"
                )
            if state["audit"] is None:
                raise CodexAuthoringContractError(
                    f"Codex v5 event audit failed: {state['event_audit_error']}"
                )
            ...
            if (
                ...
                raise CodexAuthoringContractError(
                    "Codex v5 final message is missing"
                )
            raw_final = read_stable_regular_file(
                final_message_path,
                label="Codex v5 final message",
            ...
            if raw_final not in {agent_message_bytes, agent_message_bytes + b"\n"}:
                raise CodexAuthoringContractError(
                    "Codex v5 JSONL message differs from output-last-message"
                )
        ...
        if state["raw_final_bytes"] is None and os.path.lexists(
            ...
            state["raw_final_bytes"] = read_stable_regular_file(
                final_message_path,
                label="Codex v5 staged raw final message after rejected output",
                max_bytes=authoring_input.session_budget.max_final_output_bytes,
            )
        ...
        if not terminal_process_state_is_stable(state):
            raise CodexAuthoringContractError(
                "Codex v5 process state is not terminal; guard remains authoritative"
            )
