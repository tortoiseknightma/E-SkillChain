"""Build the create-only Codex/high v5 authoring approval package.

v5 preserves v4 as a consumed, rejected real session and binds that complete
canonical bundle as the reason for an explicit prompt-contract repair.  This
builder never creates authority, claim, or output files and performs no
inference.
"""
...
import scripts.build_codex_authoring_approval_package_v4 as v4_builder
import scripts.run_codex_authoring_v4 as v4_runner


ROOT = Path(__file__).resolve().parents[1]
AUTHORING_ROOT = ROOT / "specs" / "authoring"
DEPLOY_ROOT = ROOT / "deploy" / "authoring" / "locks-codex-v5"
...
RUNNER_PATH = ROOT / "scripts" / "run_codex_authoring_v5.py"
V3_RUNNER_PATH = ROOT / "scripts" / "run_codex_authoring_v3.py"
V4_RUNNER_PATH = ROOT / "scripts" / "run_codex_authoring_v4.py"
BASE_RUNNER_PATH = ROOT / "scripts" / "run_codex_authoring.py"
CONTRACT_MODULE_PATH = ROOT / "src" / "skillchain" / "codex_authoring_v5.py"
V4_CONTRACT_MODULE_PATH = ROOT / "src" / "skillchain" / "codex_authoring_v4.py"
...
APPROVAL_BUILDER_PATH = ROOT / "scripts" / "approve_codex_authoring_v5.py"

PACKET_PATH = AUTHORING_ROOT / "authoring-packet-codex-high-v5.json"
SCHEMA_PATH = AUTHORING_ROOT / "authoring-content-schema-codex-high-v5.json"
REQUEST_PATH = AUTHORING_ROOT / "authoring-request-codex-high-v5.json"
STDIN_PATH = AUTHORING_ROOT / "authoring-stdin-codex-high-v5.txt"
PROTOCOL_PATH = AUTHORING_ROOT / "authoring-codex-mediation-protocol-v5.json"
BINARY_EVIDENCE_PATH = (
    AUTHORING_ROOT / "codex-cli-absolute-binary-evidence-v5.json"
)
...
FREEZE_PATH = AUTHORING_ROOT / "authoring-freeze-lock-codex-high-v5.json"
...
V4_CANDIDATE_ID = v4_builder.CODEX_AUTHOR_CANDIDATE_ID
V4_RUN_ID = v4_builder.CODEX_AUTHOR_RUN_ID
V4_FREEZE_PATH = v4_builder.FREEZE_PATH
V4_APPROVAL_PATH = v4_builder.APPROVAL_PATH
V4_CLAIM_PATH = v4_builder.CLAIM_PATH
V4_TERMINAL_GUARD_PATH = v4_builder.TERMINAL_GUARD_PATH
V4_OUTPUT_DIR = v4_builder.OUTPUT_DIR
V4_RECEIPT_PATH = v4_builder.RECEIPT_PATH
V4_RAW_OUTPUT_PATH = V4_OUTPUT_DIR / "author-content.raw.json"
...
def _verify_history() -> tuple[bytes, dict[str, object]]:
    ...
    _verified_v2_incident()
    unexpectedly_occupied = [
        path
        for path in (
            V3_APPROVAL_PATH,
            V3_CLAIM_PATH,
    ...
    if unexpectedly_occupied:
        raise ValueError(
            "pre-v5 history unexpectedly occupied: "
            + ", ".join(
                path.relative_to(ROOT).as_posix()
                for path in unexpectedly_occupied
            )
        )

    retirement_bytes = read_stable_regular_file(
        V2_RETIREMENT_CLAIM_PATH,
        label="v2 non-inference retirement claim",
    )
    retirement = json.loads(retirement_bytes)
    unsigned_retirement = dict(retirement)
    retirement_payload_sha = unsigned_retirement.pop(
        "retirement_payload_sha256",
        None,
    )
    if (
        retirement_payload_sha
        != sha256_bytes(canonical_json_bytes(unsigned_retirement))
        or retirement.get("status")
        != "authorization_retired_without_codex_process_launch"
        or retirement.get("successor_candidate_id") != V4_CANDIDATE_ID
        or retirement.get("successor_run_id") != V4_RUN_ID
        ...
        raise ValueError("v2 retirement history is not the exact v4 predecessor")

    guard_bytes = read_stable_regular_file(
        V4_TERMINAL_GUARD_PATH,
        label="v4 terminal guard",
    )
    guard_file_sha256 = sha256_bytes(guard_bytes)
    if not v4_runner.validate_canonical_bundle_from_disk(
        V4_OUTPUT_DIR,
        claim_path=V4_CLAIM_PATH,
        guard_path=V4_TERMINAL_GUARD_PATH,
        guard_file_sha256=guard_file_sha256,
    ):
        raise ValueError("v4 canonical consumed-session bundle is invalid")
    receipt_bytes = read_stable_regular_file(
        V4_RECEIPT_PATH,
        label="v4 canonical receipt",
    )
    receipt = json.loads(receipt_bytes)
    if (
        receipt.get("status") != "codex_session_completed_draft_rejected"
        or receipt.get("candidate_id") != V4_CANDIDATE_ID
        or receipt.get("run_id") != V4_RUN_ID
        or receipt.get("authorization_consumed") is not True
        or receipt.get("formal_codex_session_eligible") is not False
        or receipt.get("failure_stage") != "trusted_compiler"
        or receipt.get("process_launched") is not True
        or receipt.get("process_reaped") is not True
        or receipt.get("repository_retry_performed") is not False
        or receipt.get("followup_performed") is not False
        or receipt.get("repair_performed") is not False
        or receipt.get("fallback_performed") is not False
    ):
        raise ValueError("v4 receipt is not the expected consumed compiler rejection")
...
def _expected_files() -> dict[Path, bytes]:
    ...
    v4_receipt_bytes, v4_receipt = _verify_history()
    v2_incident_bytes, v2_incident = _verified_v2_incident()
    ...
    v3_incident_bytes = read_stable_regular_file(
        V3_AUDIT_INCIDENT_PATH,
        label="v3 preapproval audit incident",
    )
    v3_incident = json.loads(v3_incident_bytes)
    unsigned_v3_incident = dict(v3_incident)
    v3_incident_payload_sha = unsigned_v3_incident.pop(
        "incident_payload_sha256",
        None,
    )
    if (
        v3_incident_payload_sha
        != sha256_bytes(canonical_json_bytes(unsigned_v3_incident))
        or v3_incident.get("status")
        != "superseded_before_owner_approval_and_inference"
        or v3_incident.get("inference_requested") is not False
    ):
        raise ValueError("v3 preapproval audit incident history drifted")
    retirement_bytes = read_stable_regular_file(
        V2_RETIREMENT_CLAIM_PATH,
        label="v2 non-inference retirement claim",
    )
    ...
    protocol = {
        "schema_version": 1,
        "protocol_id": "authoring-codex-mediation-2026-07-24-v5",
        ...
        "prior_attempt_policy": {
            ...
            "v4_canonical_receipt": _binding(
                V4_RECEIPT_PATH,
                v4_receipt_bytes,
            ),
            "v4_status": v4_receipt["status"],
            "v4_authorization_consumed": True,
            "v4_result_rewritten_or_reclassified": False,
            "v4_failure_stage": "trusted_compiler",
            "v5_reason": (
                "make the compiler-reserved object-detector wording constraint "
        ...
        "history_policy": {
            "qwen_v3_v5_negative_results_preserved": True,
            ...
            "codex_v4_consumed_and_rejected_by_trusted_compiler": True,
            "each_v5_or_later_run_requires_a_new_freeze_and_namespace": True,
        },
    }
    protocol_bytes = _signed(protocol, "protocol_sha256")

    source_manifest_payload = build_codex_runtime_dependency_snapshot_v5(ROOT)
    source_manifest_bytes = _signed(
        source_manifest_payload,
        "source_manifest_payload_sha256",
    )
    environment_policy = build_codex_v5_environment_policy(
        binary=binary,
        parent_environment=os.environ,
    )
    runtime = {
        "schema_version": 3,
        "runtime_id": "codex-cli-author-runtime-20260724-v5",
        "status": "frozen",
    ...
    freeze = {
        ...
        "authorization": {
            ...
            "owner_standing_directive": (
                "continue independent Codex Author high runs until C1 "
                "authoring closes"
            ),
            "max_codex_exec_sessions": 1,
        ...
        "terminal_guard": {
            "created_only_after_owner_confirmation_before_v5_approval": True,
            "file": TERMINAL_GUARD_PATH.relative_to(ROOT).as_posix(),
            "expected_file_sha256": sha256_bytes(guard_bytes),
        ...
        "prior_attempt_history": {
            ...
            "v2_approval_reusable": False,
            "v4_freeze": _binding(V4_FREEZE_PATH),
            "v4_owner_approval": _binding(V4_APPROVAL_PATH),
            "v4_terminal_guard": _binding(V4_TERMINAL_GUARD_PATH),
            "v4_attempt_claim": _binding(V4_CLAIM_PATH),
            "v4_raw_output": _binding(V4_RAW_OUTPUT_PATH),
            "v4_canonical_receipt": _binding(
                V4_RECEIPT_PATH,
                v4_receipt_bytes,
            ),
            "v4_receipt_payload_sha256": v4_receipt[
        ...
        "preserved_history": {
            "qwen_v3_v5_results_rewritten": False,
            ...
            "codex_high_v4_candidate": (
                "one real session consumed; raw output preserved; trusted "
                "compiler rejected reserved wording"
            ),
        },
    }
...
def main() -> int:
    ...
    if args.check or args.check_candidate:
        ...
        if mismatches or occupied:
            ...
            raise SystemExit(
                "Codex v5 pre-approval candidate check failed: "
                + "; ".join(details)
            )
    else:
        existing = [
        ...
        if existing:
            raise SystemExit(
                "Codex v5 approval package is create-only; existing: "
                + ", ".join(existing)
            )
        for path, content in expected.items():
        ...
        json.dumps(
            {
                ...
                "v4_result_bound_as_consumed_history": True,
            },
            sort_keys=True,
        )
    )
