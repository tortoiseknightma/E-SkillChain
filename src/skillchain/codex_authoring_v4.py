"""Transactional runtime policy for the fourth Codex authoring candidate.

Candidate v4 preserves the v3 absolute-binary and deterministic-environment
boundary.  It adds a classified create-once primitive so a claim whose hard
link committed before a later exception can never be mistaken for an
...
from skillchain import codex_authoring_v3 as v3
from skillchain.codex_authoring_v3 import *  # noqa: F403
from skillchain.tools.serialization import read_stable_regular_file


CODEX_AUTHOR_CANDIDATE_ID = "authoring-codex-high-20260724-v4"
CODEX_AUTHOR_RUN_ID = "llm-static-codex-primary-20260724-high-v4"
CODEX_EXECUTION_TYPE = v3.CODEX_EXECUTION_TYPE
CODEX_EVIDENCE_TIER = v3.CODEX_EVIDENCE_TIER
CODEX_COMMAND_SHAPE = v3.CODEX_COMMAND_SHAPE
CODEX_FROZEN_BINARY_PATH = v3.CODEX_FROZEN_BINARY_PATH
CODEX_FROZEN_BINARY_BYTES = v3.CODEX_FROZEN_BINARY_BYTES
CODEX_FROZEN_BINARY_SHA256 = v3.CODEX_FROZEN_BINARY_SHA256
CODEX_FROZEN_CLI_VERSION = v3.CODEX_FROZEN_CLI_VERSION
CODEX_INHERITED_ENV_NAMES = v3.CODEX_INHERITED_ENV_NAMES
CODEX_CONSTRUCTED_ENV_NAMES = v3.CODEX_CONSTRUCTED_ENV_NAMES
CODEX_ENV_POLICY_VERSION = v3.CODEX_ENV_POLICY_VERSION

CODEX_V4_TRUSTED_SOURCE_FILES = (
...
def build_codex_runtime_dependency_snapshot_v4(
    root: str | Path,
) -> dict[str, object]:
    """Extend the v3 dependency closure with the immutable v4 sources."""

    repository = Path(root).resolve(strict=True)
    snapshot = v3.build_codex_runtime_dependency_snapshot_v3(repository)
    trusted = list(snapshot["trusted_sources"])
    known = {item["file"] for item in trusted}
    for relative in CODEX_V4_TRUSTED_SOURCE_FILES:
        if relative in known:
            raise v3.CodexAuthoringContractError(
                f"duplicate Codex v4 trusted source: {relative}"
            )
        path = repository / relative
        content = read_stable_regular_file(
            path,
            label=f"Codex v4 trusted source {relative}",
        )
        trusted.append(
            {
                "file": relative,
                "bytes": len(content),
                "file_sha256": v3.sha256_bytes(content),
            }
        )
    snapshot["schema_version"] = 4
    snapshot["snapshot_policy"] = (
        "v3_runtime_closure_plus_v4_transactional_claim_and_receipt_sources"
    )
...
CodexAuthoringContractError = v3.CodexAuthoringContractError
build_codex_authoring_input = v3.build_codex_authoring_input
build_codex_authoring_request = v3.build_codex_authoring_request
build_codex_output_contract = v3.build_codex_output_contract
build_codex_v4_environment_policy = v3.build_codex_v3_environment_policy
construct_codex_v4_environment = v3.construct_codex_v3_environment
freeze_codex_binary_identity = v3.freeze_codex_binary_identity
load_codex_authoring_input = v3.load_codex_authoring_input
load_codex_model_access_evidence = v3.load_codex_model_access_evidence
normalize_codex_authoring_output = v3.normalize_codex_authoring_output
parse_codex_authoring_request = v3.parse_codex_authoring_request
render_codex_authoring_stdin = v3.render_codex_authoring_stdin
resolve_frozen_codex_binary = v3.resolve_frozen_codex_binary
validate_codex_cli_output_schema = v3.validate_codex_cli_output_schema
